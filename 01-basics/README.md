...
<details>
  <summary>48. Что такое HTML‑entities и зачем они нужны?</summary>
  
  HTML‑entities (сущности) — специальные последовательности для отображения символов, имеющих особое значение или отсутствующих в стандарте ASCII. Позволяют выводить на странице спецсимволы, управляющие знаки.

</details>

<details>
  <summary>49. Как выглядит запись entity в HTML?</summary>
  
  Любая entity начинается с знака амперсанда `&amp;` и заканчивается точкой с запятой `;`. Пример: `&amp;lt;` для `&lt;`, `&amp;copy;` для ©.
</details>

<details>
  <summary>50. Приведите примеры популярных HTML‑entities</summary>
...
<details>
  <summary>51. Для чего используется &amp;nbsp;?</summary>
  
  `&amp;nbsp;` — неразрывный пробел, применяется для неделимого пробела между словами/символами (не разрывается при переносе строки).
</details>

<details>
  <summary>52. Можно ли вывести в браузере сам символ `&lt;` или `&gt;`?</summary>
  
  Да, но для корректного HTML нужно использовать `&amp;lt;` и `&amp;gt;`, иначе браузер воспримет их как начало/конец тега.
</details>

<details>
  <summary>53. Как добавить комментарий в HTML?</summary>
  
  Комментарий пишется между `&lt;!--` и `--&gt;`. Пример: `&lt;!-- Это комментарий --&gt;`.
</details>


<details>
  <summary>54. Влияют ли комментарии на отображение страницы?</summary>
  
  Нет, содержимое комментариев не отображается в браузере, но может влиять на размер страницы (передаётся клиенту).
</details>

<details>
  <summary>55. Можно ли размещать комментарии внутри других тегов?</summary>
  
  Да, комментарии можно вставлять в любом месте HTML-документа (кроме определённых мест, например, внутри атрибута).
</details>

<details>
  <summary>56. Как использовать комментарии для удобства разработки?</summary>
  
  Комментарии помогают структурировать код, отмечать блоки, TODO, отключать части разметки без удаления, облегчая коллективную работу.
</details>